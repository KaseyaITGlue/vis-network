...
class CanvasRenderer {
  ...
  _drawNodes(ctx, alwaysShow = false) {
    ...
    for (let i = 0; i < nodeIndices.length; i++) {
      ...
      if (node.options.zIndex) { // ignore 0
    ...
    const zIndices = Object.keys(nodeIndicesByZIndex).sort();

    // draw nodes with zIndex smaller than 0
    ...
    for (const index of zIndices) {
      if (index > 0) continue;
      for (const i of nodeIndicesByZIndex[index]) {
        const node = nodes[i];
        node.draw(ctx);
      }
    }
